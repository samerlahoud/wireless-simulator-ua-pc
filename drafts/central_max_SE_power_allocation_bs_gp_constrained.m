...
max_power_vector=10.*netconfig.max_power_per_RB*ones(nb_sectors,1);
...
for i=1:total_nb_users
    ...
    for j=1:nb_sectors
        if any(BS(j).attached_users == i)
            sinr(i)=sinr(i)+log((power_allocation_vector(j)*pathloss_matrix(i,j))/(noise_density + interference(i,j)));           
        end
    end 
end
...
min_power_vector <= power_allocation_vector 
power_allocation_vector <= max_power_vector
for i=1:total_nb_users
    sinr(i) >= user_sinr(i) 
end
% for j=1:nb_sectors
