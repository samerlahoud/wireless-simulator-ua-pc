...
for t_ = 1:length(BS_type)
    ...
    elseif ismember(BS_type(t_),'femto')
        ...
        netconfig.nb_mmwave_BSs = floor(BS_type_count(t_)*mmwave_BS_percentage);
        netconfig.nb_femto_BSs = BS_type_count(t_)-netconfig.nb_mmwave_BSs;
    end
end
...
% Generate femto and mmwave BS indices
randomized_rel_index = randperm(nb_BSs - nb_macro_BSs);
femto_BS_index = nb_macro_BSs + randomized_rel_index(1:nb_femto_BSs);
mmwave_BS_index = nb_macro_BSs + randomized_rel_index(nb_femto_BSs+1:end);

% Reorder coordinates for one mmwave and one femto per sector
BS_abs=[BS_abs(1:nb_macro_BSs);BS_abs(femto_BS_index);BS_abs(mmwave_BS_index)];
BS_ord=[BS_ord(1:nb_macro_BSs);BS_ord(femto_BS_index);BS_ord(mmwave_BS_index)];
...
for u = 1:nb_users
    ...
    for b = nb_macro_BSs+1:nb_macro_femto_BSs
        reordered_index = femto_BS_index(b-nb_macro_BSs);
