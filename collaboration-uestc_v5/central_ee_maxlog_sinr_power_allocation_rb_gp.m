...
function [power_allocation_matrix, sinr_matrix] = central_ee_maxlog_sinr_power_allocation_rb_gp(netconfig, pathloss_matrix, BS)
    ...
    for j=1:nb_sectors
        for i=BS(j).attached_users
            for k=1:nb_RBs
                ...
                sinr(i,j,k)= (power_allocation_matrix(j,k)*pathloss_matrix(i,j,k))/(noise_density + interference(i,j,k));
