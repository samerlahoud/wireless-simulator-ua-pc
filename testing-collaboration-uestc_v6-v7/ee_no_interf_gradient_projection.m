...
power_allocation_matrix=rand(nb_sectors,nb_RBs)*5;
...
while(1) 
        ...
        for j=1:nb_sectors
            for k=1:nb_RBs
                power_allocation_matrix(j,k) = power_allocation_matrix(j,k) + ...
                    delta*(nb_users_per_sector/power_allocation_matrix(j,k) - eta * power_prop_coeff);
    ...
    for j=1:nb_sectors
        for i=eNodeBs(j).attached_users
            for k=1:nb_RBs
                sinr= (power_allocation_matrix(j,k)*pathloss_matrix(i,j,k))/(noise_density*RB_bandwidth);
