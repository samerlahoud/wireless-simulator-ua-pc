function [power_allocation_matrix, sinr_matrix, time_structure] = distributed_gt_ee_maxlog_sinr_power_allocation_gradient(pathloss_matrix, BS)
...
while(1)
    ...
    for j=1:nb_sectors
        % converge is faster if we keep the parameter from last ietration
        %dinkelbach_param = netconfig.starting_dinkelbach_param;
        % EE for each sector
        while(1)
            for s=0:nb_steps
...
save('./output/1sector-is-500-user-100/distributed_gt_ee_maxlog_power_iteration.mat', 'cumulative_power_allocation_matrix');
