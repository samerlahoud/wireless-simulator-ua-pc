...
reuse_min_pathloss_vector = [1e11, 1e12, 1e13];
cum_femto_size = [];
cum_femto_demand = []; 
...
for i = 1:nb_iterations 
    for j = 1:length(reuse_min_pathloss_vector)
        ...
        load(sprintf('./output/user-association-output/rb-ua-100users-uniform-0.25-var-cluster/rb-ua-100users-uniform-0.25-%ddB/radio-conditions-100users-%drun.mat',10*log10(reuse_min_pathloss_vector(j)), i));
...
f=figure;
boxplot(cum_femto_size','notch', 'off', 'Label', {'110dB', '120dB', '130dB'});
...
f=figure;
boxplot(cum_femto_demand','notch', 'off', 'Label', {'110dB', '120dB', '130dB'});
ylabel('Femto demand');
xlabel('Pathloss threshold');
