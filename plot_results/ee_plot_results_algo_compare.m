function ee_plot_results_algo_compare
...
for i=1:netconfig.nb_iterations
    load(sprintf('./output/1sector-is-500-user-200/5users/results-compare-se-ee-%dusers-%dsectors-%dRBs-%.1fW-%dW-%drun.mat', ...
...
%     'notch', 'off', 'Label', {'Central-EE', 'Distributed-EE', 'Central-SE', 'Min-Power', 'Max-Power',  'EE-NoInterference'});
% ylabel('Energy Efficiency');
...
f=figure;
boxplot([cum_ee_objective, cum_gt_objective, cum_maxlog_objective, cum_pmax_objective, cum_pmin_objective, cum_ee_nointerf_objective],...
    'notch', 'off');
set(gca,'xtick',1:6, 'xticklabel',{'Central-EE', 'Distributed-EE', 'Central-SE', 'Max-Power', 'Min-Power', 'EE-NoInterference'})
ylabel('Energy Efficiency');
cleanfigure;
print(f,'-depsc', sprintf('./output/heur-boxplot-objective%s.eps', figure_file_name));
savefig(sprintf('./output/heur-boxplot-objective%s.fig', figure_file_name));
...
f=figure;
boxplot([cum_ee_spectral_eff, cum_gt_spectral_eff, cum_maxlog_spectral_eff, cum_pmax_spectral_eff, ...
    cum_pmin_spectral_eff, cum_ee_nointerf_spectral_eff],'notch', 'off');
set(gca,'xtick',1:6, 'xticklabel',{'Central-EE', 'Distributed-EE', 'Central-SE', 'Max-Power', 'Min-Power', 'EE-NoInterference'})
...
sinr_db_min=min([cum_maxlog_sinr_db_sorted_vector' cum_ee_sinr_db_sorted_vector' cum_pmin_sinr_db_sorted_vector' ...
    cum_pmax_sinr_db_sorted_vector' cum_ee_nointerf_sinr_db_sorted_vector' cum_gt_sinr_db_sorted_vector']);
sinr_db_max=max([cum_maxlog_sinr_db_sorted_vector' cum_ee_sinr_db_sorted_vector' cum_pmin_sinr_db_sorted_vector' ...
    cum_pmax_sinr_db_sorted_vector' cum_ee_nointerf_sinr_db_sorted_vector' cum_gt_sinr_db_sorted_vector']);
binranges = sinr_db_min - mod(sinr_db_min,10):10:sinr_db_max + mod(-sinr_db_max,10);
[bincounts] = histc([cum_ee_sinr_db_sorted_vector, cum_gt_sinr_db_sorted_vector, cum_maxlog_sinr_db_sorted_vector, ...
...
legend({'Central-EE', 'Distributed-EE', 'Central-SE', 'Min-Power', 'Max-Power',  'EE-NoInterference'}, 'Location', 'NorthWest');
...
ylabel('CDF');
xlabel('SINR (dB)');
legend({'Central-EE', 'Central-SE', 'Min-Power', 'Max-Power',  'EE-NoInterference'}, 'Location', 'NorthWest');
hold off;
...
set(h(2), 'FaceColor','y', 'EdgeColor','k')
set(h(1), 'FaceColor','m', 'EdgeColor','k')
legend({'Central-EE', 'Central-SE', 'Min-Power', 'Max-Power',  'EE-NoInterference'}, 'Location', 'NorthEast');
...
set(h,'color','r','LineWidth',2)
hold on;
h=cdfplot(cum_maxlog_power_sorted_vector);
set(h,'color','b','LineWidth',2)
title('Power distribution');
ylabel('CDF');
xlabel('Power (W)');
legend({'Central-EE', 'Central-SE'}, 'Location', 'SouthEast');
print(f,'-depsc', sprintf('./output/heur-power-cdf%s.eps', figure_file_name));
savefig(sprintf('./output/heur-power-cdf%s.fig', figure_file_name));
...
ylabel('CDF');
xlabel('Power (W)');
legend({'Central-EE', 'Distributed-EE'}, 'Location', 'SouthEast');
...
set(gca,'xtick',1:5, 'xticklabel',{'Central-EE', 'Central-SE', 'Min-Power', 'Max-Power',  'EE-NoInterference'})
ylabel('SINR (dB)');
cleanfigure;
...
f=figure;
boxplot([cum_ee_sinr_db_vector cum_gt_sinr_db_vector], 'notch', 'off');
set(gca,'xtick',1:2, 'xticklabel',{'Central-EE', 'Distributed-EE'})
ylabel('SINR (dB)');
cleanfigure;
...
%     'Label', {'Central-EE', 'Central-SE', 'Min-Power', 'Max-Power',  'EE-NoInterference'});
% ylabel('Power (W)');
